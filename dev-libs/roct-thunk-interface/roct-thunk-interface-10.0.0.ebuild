...
if [[ ${PV} == *9999 ]] ; then
	...
	SRC_URI="https://github.com/ROCm/rocm-systems/releases/download/${ROCM_TAG}/rocr-runtime.tar.gz -> ${P}.tar.gz"
...
src_install() {
	cmake_src_install
	# therock-10.0's rocr-runtime amd_core_dump.cpp includes
	# hsakmt/linux/kfd_ioctl.h, but the CMake install excludes linux/.
	insinto /usr/include/hsakmt/linux
	doins include/hsakmt/linux/*.h
}
